...
namespace SGEnviro.Forecasts
{
    ...
    public class Pm25RegionalUpdate
    {
        ...
        public static Pm25RegionalUpdate FromXElement(XElement region)
        {
            ...
            float readingValue;
            Parsing.ParseFloatOrThrowException(readingElement.Attribute("value").Value, out readingValue, PARSE_PM25_ERROR);

            float latitude;
            Parsing.ParseFloatOrThrowException(region.Element("latitude").Value, out latitude, PARSE_LATITUDE_ERROR);

            float longitude;
            Parsing.ParseFloatOrThrowException(region.Element("longitude").Value, out longitude, PARSE_LONGITUDE_ERROR);

            var rawTimestamp = region.Element("record").Attribute("timestamp").Value;
            int year;
            Parsing.ParseIntOrThrowException(rawTimestamp.Substring(0, YEAR_LENGTH), out year, PARSE_YEAR_ERROR);

            int month;
            const int monthStartPosition = YEAR_LENGTH;
            Parsing.ParseIntOrThrowException(rawTimestamp.Substring(monthStartPosition, MONTH_LENGTH), out month, PARSE_MONTH_ERROR);

            int day;
            const int dayStartPosition = YEAR_LENGTH + MONTH_LENGTH;
            Parsing.ParseIntOrThrowException(rawTimestamp.Substring(dayStartPosition, DAY_LENGTH), out day, PARSE_DAY_ERROR);

            int hour;
            const int hourStartPosition = YEAR_LENGTH + MONTH_LENGTH + DAY_LENGTH;
            Parsing.ParseIntOrThrowException(rawTimestamp.Substring(hourStartPosition, HOUR_LENGTH), out hour, PARSE_HOUR_ERROR);

            int minute;
            const int minuteStartPosition = YEAR_LENGTH + MONTH_LENGTH + DAY_LENGTH + HOUR_LENGTH;
            Parsing.ParseIntOrThrowException(rawTimestamp.Substring(minuteStartPosition, MINUTE_LENGTH), out minute, PARSE_MINUTE_ERROR);

            int second;
            const int secondStartPosition = YEAR_LENGTH + MONTH_LENGTH + DAY_LENGTH + HOUR_LENGTH + MINUTE_LENGTH;
            Parsing.ParseIntOrThrowException(rawTimestamp.Substring(secondStartPosition, SECOND_LENGTH), out second, PARSE_SECOND_ERROR);
